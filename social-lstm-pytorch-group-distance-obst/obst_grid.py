...
def get_obst_grid_mask(frame, dataset, dimensions, neighborhood_size, grid_size):
    ...
    # load obstacle map
    if dataset == 0:
        obst_data = np.load("obst/normalized_sampled/eth_univ_obst.npy")
    elif dataset == 1:
        obst_data = np.load("obst/normalized_sampled/hotel_obst.npy")
    elif dataset == 2:
        obst_data = np.load("obst/normalized_sampled/zara01_obst.npy")
    elif dataset == 3:
        obst_data = np.load("obst/normalized_sampled/zara02_obst.npy")
    else:
        obst_data = np.load("obst/normalized_sampled/ucy_univ_obst.npy")
    ...
    for pedindex in range(mnp):
        ...
        for obst_index in range(mno):
            ...
            frame_mask[pedindex, obst_index, cell_x + cell_y * grid_size] = distance(current_x, current_y,
                                                                                     obst_x, obst_y)
    return frame_mask
