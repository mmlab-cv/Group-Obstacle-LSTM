...
class SocialLSTM(nn.Module):
    ...
    def __init__(self, args, infer=False):
        ...
        # Linear layer to embed the obstacle tensor
        self.obst_tensor_embedding_layer = nn.Linear(self.grid_size * self.grid_size * self.max_obst,
                                                     self.embedding_size)
    ...
    def getObstacleTensor(self, grid):
        '''
        Computes the Obstacle tensor similar to the social tensor
        '''
        numNodes = grid.size()[0]
        numObst = grid.size()[1]
        numGrids = grid.size()[2]
        to_add = abs(self.max_obst - numObst)
        obst_tensor = Variable(torch.ones(numNodes, numObst, numGrids).cuda())
        for node in range(numNodes):
            obst_tensor[node] = obst_tensor[node] * grid[node]
        obst_tensor = obst_tensor.view(numNodes, numObst * numGrids)
        # concatenate the difference to make the obst tensor of the required shape
        if to_add != 0:
            pad = Variable(torch.zeros(numNodes, to_add, numGrids).cuda())
            pad = pad.view(numNodes, to_add * numGrids)
            obst_tensor = torch.cat((obst_tensor, pad), 1).cuda()
        return obst_tensor

    def forward(self, nodes, grids, obst_grids, nodesPresent, hidden_states, cell_states):
        '''
        Forward pass for the model
        params:
        nodes: Input positions
        grids: Grid masks
        obst_grids : Obstacle grid masks
        ...
        for framenum in range(self.seq_length):
            ...
            obst_grid_current = obst_grids[framenum]
            ...
            obst_tensor = self.getObstacleTensor(obst_grid_current)
            # Embed inputs
            ...
            # Embed obst tensor
            obst_tensor_embedded = self.dropout(self.relu(self.obst_tensor_embedding_layer(obst_tensor)))
            # Concat input
            concat_embedded = torch.cat((input_embedded, tensor_embedded, obst_tensor_embedded), 1)
