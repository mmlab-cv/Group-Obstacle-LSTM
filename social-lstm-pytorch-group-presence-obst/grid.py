...
def getGridMask(frame, dimensions, neighborhood_size, grid_size):
    ...
    width_bound, height_bound = (neighborhood_size/(width*1.0))*4, (neighborhood_size/(height*1.0))*4

    # For each ped in the frame (existent and non-existent)
    for pedindex in range(mnp):

        # Get x and y of the current ped
        current_x, current_y, current_id = frame[pedindex, 1], frame[pedindex, 2], frame[pedindex, 3]
        ...
        for otherpedindex in range(mnp):
            ...
            other_x, other_y, other_id= frame[otherpedindex, 1], frame[otherpedindex, 2], frame[otherpedindex, 3]
